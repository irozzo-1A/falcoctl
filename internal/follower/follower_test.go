...
func TestStartupBehaviorSkip(t *testing.T) {
	printer := output.NewPrinter(pterm.LogLevelDebug, pterm.LogFormatterJSON, os.Stdout)
	...
	// Create a schedule that will trigger in the future
	sched, err := cron.ParseStandard("*/5 * * * *") // Every 5 minutes
	assert.NoError(t, err)
	...
	// Verify that startupBehavior returns Skip
	assert.Equal(t, StartupBehaviorSkip, f.startupBehavior())
...
func TestSleepWithContext(t *testing.T) {
	...
	t.Run("returns error when context is cancelled", func(t *testing.T) {
		...
		assert.Less(t, elapsed, duration, "should return early when context is cancelled")
	})
